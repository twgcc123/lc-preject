			...
			<view class="pay-type">
				<view class="pay-item" v-for="(item, index) in payList" :key="index">
					...
					<view class="check" :class="['check', { checkedbox: index == activeIndex }]" @tap="checked(item, index)" type="radio">
						<uni-icons type="checkmarkempty" color="#3F3F3F 100%" size="18"></uni-icons>
					</view>
				</view>
			</view>
			<!-- <view class="line"></view> -->
		</view>
		<view class="btnPay" @tap="surPay('/pages/found/youju/roomOrderPaySuccess')">确定支付 {{ orderPayPrice | balanceFil }}</view>
	</view>
</template>
