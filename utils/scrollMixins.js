...
export default {
	data(){
		return{
			...
			oldIndex:0,
			width:'',
			sysheight:uni.getStorageSync('sysheight'),
			distance:uni.getStorageSync('distance'),
			distanceX:0,
			scroll:false,
		}
	},
	...
	methods:{
				...
				props: [
					...
					if(e.state === 'end' || e.state === 'exit') {
						...
						if( Math.abs(Y)>this.backDistance||quickMove) {
							...
							uni.setStorageSync('sysheight', this.sysheight);							
		...
		bindTimingX(distance,X){
			console.log('执行动画2')
			// 开始执行动画
			console.log("obindTimingX --> distance:",distance,'-->X:',X)
			this.scroll = true
			let swiperRef = this.getEl(this.$refs.swiper)
			let leftRef = this.getEl(this.$refs.left)
			let rightRef = this.getEl(this.$refs.right)
