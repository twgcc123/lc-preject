<template>
  <view class="dropdown-menu">
    ...
    <view class="content-list"
      ...
      <view :class="['options-list', active?'active-options-list':'']"
        :style="{top: optionsHeight>0?'0px':'-'+ 200+'px'}">
        <view v-for="(item, index) in options"
          :key="index"
          :class="['option-item', value===item.value?'active-option-item':'']"
          @click="changeItem(item,index)">
          <text class="text">
            {{item.text}}
          </text>
          <view v-if="value===item.value"
            class="iconfont icon-check"></view>
        </view>
      </view>
      <view :class="['mask', active?'show-mask':'']"
        @touchmove.stop.prevent="moveHandle"
        @click="hideOptionsList">
      </view>
    </view>
  </view>
</template>
